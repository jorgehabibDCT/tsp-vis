...
/** Same logic as `server/src/utils/providerOpportunityScore.ts`. */
export function applyOpportunityScoresToPayload(
  ...
  for (const tsp of payload.tsps) {
    if (tsp.integrationStatus === 'pending_integration') {
      riskMetric.values[tsp.id] = { kind: 'scalar', value: null }
      continue
    }

    const conf =
      tsp.providerMappingConfidence ??
      TSP_PROVIDER_META[tsp.id]?.providerMappingConfidence
